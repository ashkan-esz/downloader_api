...
export async function saveLinksSample(sourceData, fieldName, replace = false) {
    try {
        ...
        for (let i = 0; i < data.downloadLinks.length; i++) {
            delete data.downloadLinks[i].pageLink;
            delete data.downloadLinks[i].sourceName;
            delete data.downloadLinks[i].qualitySample;
        }
        for (let i = 0; i < data.watchOnlineLinks.length; i++) {
            delete data.watchOnlineLinks[i].pageLink;
            delete data.watchOnlineLinks[i].sourceName;
        }
        for (let i = 0; i < data.subtitles.length; i++) {
            delete data.subtitles[i].pageLink;
            delete data.subtitles[i].sourceName;
        }
...
export async function checkPrevDataWithNewMethod(sourceName = null, updateData = false) {
    ...
    try {
        ...
        let titles = await getLinksSamples2(sourceFileName);
        let counter = 0;
            ...
            if (
                ...
                if (updateData) {
                    ...
                    if (answers.ans.toLowerCase() === 'y') {
                        await updateSampleData(titles[i], newTitle, newYear);
                    }
                }
                console.log('-------------------------');
            }
...
export async function getLinksSamples(sourceNames = null) {
    try {
        await waitForFileClose();
        isFileOpen = true;
        ...
        for (let i = 0; i < samples.length; i++) {
            for (let j = 0; j < samples[i].downloadLinks.length; j++) {
                if (samples[i].downloadLinks[j].qualitySample === undefined) {
                    samples[i].downloadLinks[j].qualitySample = "";
                }
            }
        }
        isFileOpen = false;
